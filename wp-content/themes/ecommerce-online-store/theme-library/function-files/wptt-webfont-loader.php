...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $ecommerce_online_store_url The remote URL.
		 */
		public function __construct( $ecommerce_online_store_url = '' ) {
			$this->remote_url = $ecommerce_online_store_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $ecommerce_online_store_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $ecommerce_online_store_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $ecommerce_online_store_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $ecommerce_online_store_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $ecommerce_online_store_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $ecommerce_online_store_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $ecommerce_online_store_format = 'woff2' ) {
			$this->font_format = $ecommerce_online_store_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $ecommerce_online_store_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $ecommerce_online_store_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $ecommerce_online_store_url, '/' ) ) {
				$ecommerce_online_store_parsed_url = parse_url( $this->remote_url );
				return $ecommerce_online_store_parsed_url['scheme'] . '://' . $ecommerce_online_store_parsed_url['hostname'] . $ecommerce_online_store_url;
			}

			return $ecommerce_online_store_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $ecommerce_online_store_url    The URL of the remote webfont.
	 * @param string $ecommerce_online_store_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $ecommerce_online_store_url, $ecommerce_online_store_format = 'woff2' ) {
		$ecommerce_online_store_font = new WPTT_WebFont_Loader( $ecommerce_online_store_url );
		$ecommerce_online_store_font->set_font_format( $ecommerce_online_store_format );
		return $ecommerce_online_store_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $ecommerce_online_store_url, $ecommerce_online_store_format = 'woff2' ) {
		$ecommerce_online_store_font = new WPTT_WebFont_Loader( $ecommerce_online_store_url );
		$ecommerce_online_store_font->set_font_format( $ecommerce_online_store_format );
		return $ecommerce_online_store_font->get_url();
	}
}